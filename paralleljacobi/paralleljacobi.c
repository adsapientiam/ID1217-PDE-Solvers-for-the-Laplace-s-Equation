// Parallel Jacobi method for solving linear systems of equations
...
double jacobiIteration(int gridInteriorSize, double (*grid)[gridInteriorSize + 2], double (*newGrid)[gridInteriorSize + 2], int numIters) {
    ...
    for (int k = 0; k < numIters; k++) {
        maxDiff = 0.0;
        #pragma omp parallel for reduction(max:maxDiff) private(diff) schedule(static)
        ...
        // Swap the grids (avoids having to do two jacobi iterations per loop iteration)
...
int main(int argc, char *argv[]) {
    const struct args parsedArgs = parseArgs(argc, argv);
